...
public class DuplicatesScreenLeftCellFactory<T> implements Callback<ListView<T>, ListCell<Tag>> {
  ...
  public DuplicatesScreenLeftCellFactory(Supplier<SimilarPair<MediaItem>> currentPair) {
    this.currentPair = currentPair;
  }

  @Override
  public TagListCell call(ListView<T> tListView) {
    TagListCell c = new OtherMissingTagListCell(() -> currentPair.get().getObject2());
    MenuItem addToOther = new MenuItem("Add to other");
    addToOther.setOnAction(event -> currentPair.get().getObject2().addTag(c.getItem()));
    MenuItem removeTag = new MenuItem("Remove tag");
    removeTag.setOnAction(event -> currentPair.get().getObject1().removeTag(c.getItem()));
